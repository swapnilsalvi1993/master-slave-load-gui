...
class CSVPlotter:
    ...
    def create_widgets(self):
        ...
        # Legend positions numeric entries - fixed width numeric boxes
        ...
        # Place Right Legend X and Y adjacent so they're visible without wide horizontal scrolling
        ...
        # Use same fixed numeric width for axis limit entries
    ...
    def reset_axis_colors(self):
        self.left_axis_color.set("#000000"); self.right_axis_color.set("#1f77b4"); self.right2_axis_color.set("#d62728")
        try:
    ...
    def choose_marker_color(self, row_frame, swatch_label):
        ...
        if color_code:
            ...
            try: swatch_label.configure(background=color_code)
    ...
    def browse_file(self):
        ...
        if file_path:
            self.file_entry.delete(0, "end"); self.file_entry.insert(0, file_path)
    ...
    def load_csv(self, file_path):
        ...
        self.y_listbox.delete(0, "end"); self.right_y_listbox.delete(0, "end"); self.right2_y_listbox.delete(0, "end")
        for col in self.df.columns:
            self.y_listbox.insert("end", col)
            self.right_y_listbox.insert("end", col)
            self.right2_y_listbox.insert("end", col)
        # clear property frames (they will be rebuilt when selections change)
        self.reset_line_properties(); self.reset_right_line_properties(); self.reset_right2_line_properties()
        self.schedule_preview()
    ...
    def move_selected_in_listbox(self, listbox, direction=1):
        try:
            ...
            for it in items: listbox.insert(tk.END, it)
            listbox.selection_clear(0, tk.END)
            for i in new_sel: listbox.selection_set(i)
    ...
    def update_line_properties(self):
        ...
        for col in y_cols:
            row = ttk.Frame(self.line_prop_frame); row.pack(fill="x", pady=1, padx=2)
            row._col_name = col
            ttk.Label(row, text=col, width=20).pack(side="left", padx=2)
            label_var = tk.StringVar(value=col)
            label_entry = ttk.Entry(row, textvariable=label_var, width=30); label_entry.pack(side="left", padx=4)
            ...
            style_cb = ttk.Combobox(row, values=["-","--","-.",":"], textvariable=style_var, width=4); style_cb.pack(side="left")
            ...
            marker_cb = ttk.Combobox(row, values=["None","o","s","^","*"], textvariable=marker_var, width=4); marker_cb.pack(side="left")
            marker_cb.bind("<<ComboboxSelected>>", lambda e: (self.sync_channel_props_from_frames(), self.schedule_preview()))
            color = next(self.color_cycle)
            swatch = tk.Label(row, background=color, width=2, relief="sunken"); swatch.pack(side="left", padx=(6,2))
            ttk.Button(row, text="Choose", command=lambda r=row, s=swatch: self.choose_color(r, s)).pack(side="left", padx=(2,6))
            marker_color_swatch = tk.Label(row, background=color, width=2, relief="raised"); marker_color_swatch.pack(side="left", padx=(4,2))
            row.marker_color = ""; row._marker_swatch = marker_color_swatch
            ...
            row.line_color = color; row.line_style = style_var; row.marker = marker_var; row._linked_label_var = label_var; row._swatch = swatch
    ...
    def update_right_line_properties(self):
        ...
        for col in right_cols:
            row = ttk.Frame(self.right_line_prop_frame); row.pack(fill="x", pady=1, padx=2)
            row._col_name = col
            ttk.Label(row, text=col, width=20).pack(side="left", padx=2)
            label_var = tk.StringVar(value=col)
            label_entry = ttk.Entry(row, textvariable=label_var, width=30); label_entry.pack(side="left", padx=4)
    ...
    def update_right2_line_properties(self):
        ...
        for col in right2_cols:
            row = ttk.Frame(self.right2_line_prop_frame); row.pack(fill="x", pady=1, padx=2)
            row._col_name = col
            ttk.Label(row, text=col, width=20).pack(side="left", padx=2)
            label_var = tk.StringVar(value=col)
            label_entry = ttk.Entry(row, textvariable=label_var, width=30); label_entry.pack(side="left", padx=4)
    ...
    def schedule_preview(self):
        if not self.live_preview_var.get(): return
        if self._preview_after_id is not None:
            try: self.root.after_cancel(self._preview_after_id)
            except Exception: pass
    ...
    def preview_plot(self):
        ...
        if self.df is None:
            try: self.fig.clf(); self.canvas.draw()
            except Exception: pass
            return
        ...
        for i, col in enumerate(y_cols):
            ...
            if i < len(self.line_properties_frames):
                try:
                    ...
                    if var.get().strip(): label = var.get().strip()
                except Exception: pass
            labels.append(label)
        ...
        try:
            pw = float(self.plot_width.get()); ph = float(self.plot_height.get())
        except Exception:
            pw = self.default_plot_width; ph = self.default_plot_height
        ...
        if right_cols:
            ...
            try: ax2.set_ylabel(self.right_ylabel_entry.get() or ", ".join(right_labels), fontsize=self.font_size.get())
            except Exception: pass
        ...
        if right2_cols:
            ax3 = self.ax.twinx()
            try:
                pos = float(self.right2_pos.get()); ax3.spines["right"].set_position(("axes", pos))
            except Exception: pass
            try: ax3.set_frame_on(True)
            except Exception: pass
            ...
            try: ax3.set_ylabel(self.right2_ylabel_entry.get() or ", ".join(right2_labels), fontsize=self.font_size.get())
            except Exception: pass
        ...
        try:
            if self.xinterval_entry.get().strip():
                ...
                if xint > 0: self.ax.xaxis.set_major_locator(MultipleLocator(xint))
            if self.yinterval_entry.get().strip():
                yint = float(self.yinterval_entry.get());
                if yint > 0: self.ax.yaxis.set_major_locator(MultipleLocator(yint))
            if ax2 and self.right_yinterval_entry.get().strip():
                ryint = float(self.right_yinterval_entry.get());
                if ryint > 0: ax2.yaxis.set_major_locator(MultipleLocator(ryint))
            if ax3 and self.right2_yinterval_entry.get().strip():
                ry2int = float(self.right2_yinterval_entry.get());
                if ry2int > 0: ax3.yaxis.set_major_locator(MultipleLocator(ry2int))
        except Exception:
            pass

        try:
            if ax3 is not None: right_margin = 0.70
            elif ax2 is not None: right_margin = 0.82
            else: right_margin = 0.92
        ...
        try:
            ...
            if ax2: ax2.tick_params(axis="both", labelsize=fs)
            if ax3: ax3.tick_params(axis="both", labelsize=fs)
            if self.grid_var.get(): self.ax.grid(True)
        except Exception:
            pass

        def place_legend(ax, legend_pos, cols, x_override=None, y_override=None):
            outside = False
            loc_map = {"outside top": "upper center", "outside bottom": "lower center", "outside left": "center left", "outside right": "center right"}
            if legend_pos and legend_pos.startswith("outside"):
                outside = True; loc = loc_map.get(legend_pos, "best")
            else:
        ...
        try:
            ...
            if ax2: place_legend(ax2, self.legend_loc_right.get(), self.legend_cols_right.get(), self.legend_x_right.get(), self.legend_y_right.get())
            if ax3: place_legend(ax3, self.legend_loc_right2.get(), self.legend_cols_right2.get(), self.legend_x_right2.get(), self.legend_y_right2.get())
        except Exception:
            pass
    ...
    def save_configuration(self):
        ...
        config['legend_x_left'] = self.legend_x_left.get(); config['legend_y_left'] = self.legend_y_left.get()
        config['legend_x_right'] = self.legend_x_right.get(); config['legend_y_right'] = self.legend_y_right.get()
        config['legend_x_right2'] = self.legend_x_right2.get(); config['legend_y_right2'] = self.legend_y_right2.get()
        config['font_size'] = int(self.font_size.get()); config['show_grid'] = bool(self.grid_var.get())
        config['live_preview'] = bool(self.live_preview_var.get()); config['downsample_live'] = bool(self.downsample_live_var.get())
        config['max_preview_points'] = int(self.max_preview_points.get()); config['marker_size'] = float(self.marker_size.get())
        config['plot_width'] = float(self.plot_width.get()); config['plot_height'] = float(self.plot_height.get()); config['global_line_width'] = float(self.global_line_width.get())
        config['axis_colors'] = {'left': self.left_axis_color.get(), 'right': self.right_axis_color.get(), 'right2': self.right2_axis_color.get()}
        ...
        config['axis_limits'] = {
            'xmin': self.xmin_entry.get(), 'xmax': self.xmax_entry.get(),
            'ymin': self.ymin_entry.get(), 'ymax': self.ymax_entry.get(),
            'right_ymin': self.right_ymin_entry.get(), 'right_ymax': self.right_ymax_entry.get(),
            'right2_ymin': self.right2_ymin_entry.get(), 'right2_ymax': self.right2_ymax_entry.get()
        }
    ...
    def load_configuration(self):
        ...
        try:
            ...
            try: self.font_size.set(cfg.get('font_size', self.font_size.get()))
            except Exception: pass
            ...
            try: self.max_preview_points.set(int(cfg.get('max_preview_points', self.max_preview_points.get())))
            except Exception: pass
            try: self.marker_size.set(float(cfg.get('marker_size', self.marker_size.get())))
            except Exception: pass
            try:
            ...
            try: self.global_line_width.set(float(cfg.get('global_line_width', self.global_line_width.get())))
            except Exception: pass
            ...
            for key, entry_widget in (('xmin', self.xmin_entry), ('xmax', self.xmax_entry),
                ...
                try:
                    ...
                    if val is not None: entry_widget.insert(0, str(val))
                except Exception: pass

            # restore detected header index if present
            try: self.detected_header_line_index = cfg.get('detected_header_line_index', None)
            except Exception: self.detected_header_line_index = None

            # populate persistent maps (we don't set selections here)
            try:
                ...
                for p in cfg.get('left_channel_props', []):
                    ...
                    if not col: continue
                    self.left_channel_props[col] = {
                        "label": p.get('label', col), "line_color": p.get('line_color', ""), "marker_color": p.get('marker_color', "") or "",
                        "style": p.get('style', "-"), "marker": p.get('marker', "None")
                    }
            except Exception: pass
            try:
    ...
    def save_png(self):
        ...
        try: pw = float(self.plot_width.get()); ph = float(self.plot_height.get())
        except Exception: pw = self.default_plot_width; ph = self.default_plot_height
        ...
        try: global_lw = float(self.global_line_width.get())
        except Exception: global_lw = 1.0
        ...
        for y_col, row in zip(y_cols, self.line_properties_frames):
            try:
                ...
                mcolor = getattr(row, "marker_color", "") or row.line_color
                ax_save.plot(self.df[x_col].values, ydata, linestyle=row.line_style.get(), linewidth=global_lw,
                             color=row.line_color, marker=marker, markersize=self.marker_size.get(),
                             markerfacecolor=mcolor, markeredgecolor=mcolor, label=row._linked_label_var.get())
            except Exception: continue

        try:
            left_col = self.left_axis_color.get()
            ax_save.spines["left"].set_color(left_col); ax_save.yaxis.label.set_color(left_col); ax_save.tick_params(axis="y", colors=left_col)
        except Exception: pass
        ...
        if right_cols:
            ...
            for y_col, row in zip(right_cols, self.right_line_properties_frames):
                try:
                    ...
                    ydata = self.df[y_col].values.astype(float)
                    mcolor = getattr(row, "marker_color", "") or row.line_color
                    ax2_save.plot(self.df[x_col].values, ydata, linestyle=row.line_style.get(), linewidth=global_lw,
                                  color=row.line_color, marker=marker, markersize=self.marker_size.get(),
                                  markerfacecolor=mcolor, markeredgecolor=mcolor, label=row._linked_label_var.get())
                except Exception: continue
            try:
                right_col = self.right_axis_color.get()
                ax2_save.spines["right"].set_color(right_col); ax2_save.yaxis.label.set_color(right_col); ax2_save.tick_params(axis="y", colors=right_col)
            except Exception: pass
            try: ax2_save.set_ylabel(self.right_ylabel_entry.get() or ", ".join([r._linked_label_var.get() for r in self.right_line_properties_frames]), fontsize=self.font_size.get())
            except Exception: pass

        ax3_save = None
        if right2_cols:
            ax3_save = ax_save.twinx()
            try: pos = float(self.right2_pos.get()); ax3_save.spines["right"].set_position(("axes", pos))
            except Exception: pass
            for y_col, row in zip(right2_cols, self.right2_line_properties_frames):
                try:
                    ...
                    ydata = self.df[y_col].values.astype(float)
                    mcolor = getattr(row, "marker_color", "") or row.line_color
                    ax3_save.plot(self.df[x_col].values, ydata, linestyle=row.line_style.get(), linewidth=global_lw,
                                  color=row.line_color, marker=marker, markersize=self.marker_size.get(),
                                  markerfacecolor=mcolor, markeredgecolor=mcolor, label=row._linked_label_var.get())
                except Exception: continue
            try:
                right2_col = self.right2_axis_color.get()
                ax3_save.spines["right"].set_color(right2_col); ax3_save.yaxis.label.set_color(right2_col); ax3_save.tick_params(axis="y", colors=right2_col)
            except Exception: pass
            try: ax3_save.set_ylabel(self.right2_ylabel_entry.get() or ", ".join([r._linked_label_var.get() for r in self.right2_line_properties_frames]), fontsize=self.font_size.get())
            except Exception: pass
        ...
        try:
            ...
            ymin = float(self.ymin_entry.get()) if self.ymin_entry.get() else None
            ymax = float(self.ymax_entry.get()) if self.ymax_entry.get() else None
            ax_save.set_xlim(xmin, xmax); ax_save.set_ylim(ymin, ymax)
            if ax2_save:
                rymin = float(self.right_ymin_entry.get()) if self.right_ymin_entry.get() else None
                rymax = float(self.right_ymax_entry.get()) if self.right_ymax_entry.get() else None
        ...
        try:
            if self.xinterval_entry.get().strip():
                xint = float(self.xinterval_entry.get());
                if xint > 0: ax_save.xaxis.set_major_locator(MultipleLocator(xint))
            if self.yinterval_entry.get().strip():
                yint = float(self.yinterval_entry.get());
                if yint > 0: ax_save.yaxis.set_major_locator(MultipleLocator(yint))
            if ax2_save and self.right_yinterval_entry.get().strip():
                ryint = float(self.right_yinterval_entry.get());
                if ryint > 0: ax2_save.yaxis.set_major_locator(MultipleLocator(ryint))
            if ax3_save and self.right2_yinterval_entry.get().strip():
                ry2int = float(self.right2_yinterval_entry.get());
                if ry2int > 0: ax3_save.yaxis.set_major_locator(MultipleLocator(ry2int))
        except Exception: pass

        try:
            if ax3_save is not None: right_margin = 0.70
            elif ax2_save is not None: right_margin = 0.82
            else: right_margin = 0.92
            right_margin = max(0.5, min(0.95, right_margin))
        ...
        try:
            ...
            ax_save.set_ylabel(self.ylabel_entry.get() or ", ".join([r._linked_label_var.get() for r in self.line_properties_frames]), fontsize=fs)
            ax_save.tick_params(axis="both", labelsize=fs)
            if ax2_save: ax2_save.tick_params(axis="both", labelsize=fs)
            if ax3_save: ax3_save.tick_params(axis="both", labelsize=fs)
            if self.grid_var.get(): ax_save.grid(True)
        except Exception: pass
        ...
        try:
            ...
            if ax2_save: place_legend_save(ax2_save, self.legend_loc_right.get(), self.legend_cols_right.get(), self.legend_x_right.get(), self.legend_y_right.get())
            if ax3_save: place_legend_save(ax3_save, self.legend_loc_right2.get(), self.legend_cols_right2.get(), self.legend_x_right2.get(), self.legend_y_right2.get())
        except Exception: pass

        try: dpi = int(self.dpi_option.get())
        except Exception: dpi = 200
        try:
    ...
    def reset_all(self):
        try:
            self.df = None; self.file_entry.delete(0, "end"); self.x_combo.set(""); self.x_combo["values"] = []
            self.y_listbox.delete(0, tk.END); self.right_y_listbox.delete(0, tk.END); self.right2_y_listbox.delete(0, tk.END)
            self.reset_line_properties(); self.reset_right_line_properties(); self.reset_right2_line_properties()
            self.left_channel_props.clear(); self.right_channel_props.clear(); self.right2_channel_props.clear()
            self.detected_header_line_index = None
            self.reset_labels(); self.reset_limits(); self.reset_axis_colors()
            self.live_preview_var.set(True); self.downsample_live_var.set(True); self.max_preview_points.set(5000)
            self.global_line_width.set(1.0); self.marker_size.set(6.0)
            self.plot_width.set(self.default_plot_width); self.plot_height.set(self.default_plot_height)
            try: self.dpi_option.current(2)
            except Exception: pass
            self.schedule_preview()
        except Exception:
            pass
    ...
    def reset_csv(self):
        self.file_entry.delete(0, "end"); self.df = None; self.x_combo.set("")
        self.y_listbox.delete(0, "end"); self.right_y_listbox.delete(0, "end"); self.right2_y_listbox.delete(0, "end")
        self.reset_line_properties(); self.reset_right_line_properties(); self.reset_right2_line_properties()
        self.schedule_preview()

    def reset_axis(self):
        self.x_combo.set(""); self.y_listbox.selection_clear(0, "end"); self.reset_line_properties(); self.schedule_preview()

    def reset_right_axis(self):
        self.right_y_listbox.selection_clear(0, "end"); self.reset_right_line_properties(); self.schedule_preview()

    def reset_right2_axis(self):
        self.right2_y_listbox.selection_clear(0, "end"); self.reset_right2_line_properties(); self.schedule_preview()
    ...
    def reset_labels(self):
        try:
            self.title_entry.delete(0, "end"); self.xlabel_entry.delete(0, "end"); self.ylabel_entry.delete(0, "end")
            self.right_ylabel_entry.delete(0, "end"); self.right2_ylabel_entry.delete(0, "end")
        except Exception: pass
        try:
            self.legend_loc_left.current(0); self.legend_loc_right.current(0); self.legend_loc_right2.current(0)
            self.legend_cols_left.set(1); self.legend_cols_right.set(1); self.legend_cols_right2.set(1)
            self.legend_x_left.set(""); self.legend_y_left.set(""); self.legend_x_right.set(""); self.legend_y_right.set("")
            self.legend_x_right2.set(""); self.legend_y_right2.set("")
        except Exception: pass
        try:
            self.font_size.set(12); self.grid_var.set(True); self.marker_size.set(6.0)
            self.left_axis_color.set("#000000"); self.right_axis_color.set("#1f77b4"); self.right2_axis_color.set("#d62728"); self.right2_pos.set(1.2)
            self.left_axis_swatch.configure(background=self.left_axis_color.get()); self.right_axis_swatch.configure(background=self.right_axis_color.get()); self.right2_axis_swatch.configure(background=self.right2_axis_color.get())
        except Exception: pass
        self.schedule_preview()
